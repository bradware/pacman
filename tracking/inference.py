...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        """
        noisyDistance = observation         #estimated manhattan distance to ghost we are tracking, observation is distance passed in
        emissionModel = busters.getObservationDistribution(noisyDistance)
        #seperate probability function returns a key value pair of noisyDistance
        pacmanPosition = gameState.getPacmanPosition()
        #self.beliefs is a dictionary of states associated with a probability
        "*** YOUR CODE HERE ***"

        allPossible = util.Counter()
        if noisyDistance is None:  #means the ghost has been captured
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        "*** YOUR CODE HERE ***"
        deltaStepBeliefs = util.Counter()
        for oldPos in self.legalPositions:
            #self.setGhostPosition returns a gameState with the Ghost moved to a new position
            #gameState passed into setGhostPosition is the old gameState before new time step occurred
            #self.getPositionDistribution returns tuple of pos,prob for each pos on board taken into
            #account the new gameState
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos, prob in newPosDist.items():
                deltaStepBeliefs[newPos] += self.beliefs[oldPos] * prob
                #reason oldPos is used for self.beliefs is b/c those beliefs are based on prev time step...
                #and those pos iterating through legalPositions in old gameState...
                #newPos may not be valid in self.beliefs b/c it's based on a different gamestate


        self.beliefs = deltaStepBeliefs
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        #each particle represents a position
        self.particles = []
        factor = len(self.legalPositions) / (self.numParticles + 0.0)   #this makes the ratio a double
        count = 0
        while count < self.numParticles:
            self.particles.append(self.legalPositions[int(factor * count)]) #casting back to int, indexing inside
            #list of legalPositions based on the factor to increase count, this will uniformly distribute the
            #particles to different positions
            count += 1
    ...
    def observe(self, observation, gameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        "*** YOUR CODE HERE ***"
        #beliefs is just a converter/dummy counter object to store the particles weight due to the distance (observations)
        #and number of particles that were added uniformly to the self.particles list
        #after that util.sample uses the particles and resamples them for the next propagation of the particles
        ...
        if noisyDistance is None:
            ...
            for particle in self.particles:
                ...
                beliefs[particle] += emissionModel[trueDistance]
            ...
            if beliefs.totalCount() == 0:
                ...
                newParticles = []
                while count < self.numParticles:
                    newParticles.append(util.sample(beliefs))
                    count += 1
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        #In this method we are simply gettting a new observation util.Counter distribution from every new
        #gamestate from previous particle (position) and then after that resampling it with the new weight from
        #the new observations and then the new particle list is returning the new particle returned after the sample
        deltastepParticles = []
        for particle in self.particles:
            newObservation = self.getPositionDistribution(self.setGhostPosition(gameState, particle))
            deltastepParticles.append(util.sample(newObservation))

        self.particles = deltastepParticles
    ...
    def getBeliefDistribution(self):
        ...
        for particle in self.particles:
            newBeliefDistribution[particle] += 1

        newBeliefDistribution.normalize()
        return newBeliefDistribution
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        "*** YOUR CODE HERE ***"
        self.particles = []
        #did this to find the number of legalPositions possible if can occupy same space
        allPossibilities = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(allPossibilities)
        factor = len(allPossibilities) / (self.numParticles + 0.0)
        count = 0
        while count < self.numParticles:
            self.particles.append(allPossibilities[int(factor * count)]) #this distributes it uniformly with a
            #factor of allPossibilities list to numParticles
            count += 1
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for particle in self.particles:
            temp = 1
            for currGhost in range(self.numGhosts):
                if noisyDistances[currGhost] is not None:
                    trueDistance = util.manhattanDistance(pacmanPosition, particle[currGhost])
                    temp *= emissionModels[currGhost][trueDistance]
        ...
        if beliefs.totalCount() == 0:
            ...
            newParticles= []
            while count < self.numParticles:
                newParticles.append(util.sample(beliefs))   #maybe change to chooseFromDistribution
                count += 1
            self.particles = newParticles


        for currGhost in range(self.numGhosts):
            if noisyDistances[currGhost] is None:
                p = 0
                while p < self.numParticles:
                    self.particles[p] = self.getParticleWithGhostInJail(self.particles[p], currGhost)
                    p += 1
        ...
        for oldParticle in self.particles:
            ...
            for currGhost in range(self.numGhosts):
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle),
                                                             currGhost, self.ghostAgents[currGhost])
                newParticle[currGhost] = (util.sample(newPosDist))
